...
const Orders = ({ session, status }: {session?: Session | null, status: string}) => {
  ...
  return (
    <div className='flex flex-col w-full gap-3 sm:gap-5 h-full overflow-hidden'>
      ...
      <h1 className='font-semibold text-sm sm:text-lg text-zinc-800'>Your Orders Status</h1>
      <div className='flex flex-col gap-3 sm:gap-5 overflow-auto custom-scroll-bar'>
        {userOrderLoading ? (
          <>
                ...
                <div key={item.id} className='flex w-full'>
                  ...
                  <div className='flex flex-col justify-between px-3'>
                    <div className='flex flex-col'>
                      <h1 className='text-sm sm:text-md font-medium text-zinc-800'>{item.name}</h1>
                      <h1 className='text-xs sm:text-sm text-zinc-500'>{item.quantity}Pcs</h1>
                    </div>
                    <h1 className='text-xs sm:text-base'>P {item.price}.00</h1>
                  </div>
                </div>
              ))}
              <div className='flex flex-col mt-2'>
                <h1 className='text-sm sm:text-lg font-semibold'>Total: <span className='text-base font-normal'>P {order.cartTotal}.00</span></h1>
      ...
      <div className='lg:w-1/3 h-full bg-[#f5f5f5] rounded-lg shadow-lg p-3 sm:p-5 flex flex-col'>
      <div className='bg-main w-full p-5 rounded-lg flex flex-col gap-3 sm:gap-5'>
              <div className='flex gap-3 items-center'>
