...
import { useGetItemsPurchaseQuery, useUpdateItemStatusMutation, useLazyGetOrderQuery } from '@/store/api'
  ...
  return (
    <div ref={(instance) => {drag(instance)}} className={`mb-4 w-full overflow-hidden flex flex-col bg-[#f5f5f5] rounded-lg shadow-md p-5 ${isDragging ? "opacity-50":"opacity-100"}`}>
      ...
      <Dialog>
        ...
        <Button onClick={handleItemData} className='bg-main mt-5 hover:bg-main2 w-full'>View Order</Button>
        </DialogTrigger>
        <DialogContent>
            ...
            orders.map((order) => (
              <div key={order.id} className='flex gap-3'>
                ...
                <div className="flex flex-col gap-2 justify-between">
                  ...
                  <h1 className='text-zinc-500'>{order.quantity}x</h1>
                  </div> 
      ...
      {item.status === 'Finished' ? (
        <Button onClick={handleItemData} className='bg-green-700 mt-2 hover:bg-green-800 w-full'>Save Order</Button>
      ): item.status === 'Cancelled' ? (
        <Button onClick={handleItemData} className='bg-red-700 mt-2 hover:bg-red-800 w-full'>Cancel Order</Button>
      ) : null}
    </div>
  )
}
